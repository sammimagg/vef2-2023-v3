...
import { insertCourseToDb, insertDepartmentToDb, poolEnd, query } from '../lib/db.js';
...
async function setup() {
    ...
    for (const item of indexData) {
        ...
        const insertDept = await insertDepartmentToDb(department, false);

        if (!insertDept) {
            console.error('unable to insert department', false);
            continue;
        }
        let validInserts = 0;
        ...
        for (const course of courses) {
            const id = await insertCourseToDb(course, insertDept.id, true);
            if (id) {
              validInserts++;
            } else {
              invalidInserts++;
        ...
        console.info(
            `Create department ${item.title} with ${validInserts} courses and ${invalidInserts} invalid courses.`
        );
    }
    await poolEnd();
}
