...
export async function departmentDoesNotExistValidator(
  title: string
): Promise<boolean> {
...
export async function createDepartment(
  ...
  const createDepartment = await insertDepartmentToDb(
    departmentToCreate,
    false
  );
  if (!createDepartment) {
...
export async function updateDepartment(
  ...
  const result = await conditionalUpdateInDp(
    "department",
    department.id,
    fields.filter(Boolean) as string[],
